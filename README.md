...
- [Bonsai Integration and Edge AI Extension](#bonsai-integration-and-edge-ai-extension)
...
# Bonsai Integration and Edge AI Extension

This section extends JFXLCDP's specification-driven, MBSE, Modelica, low-code and AI architecture with two independent Bonsai building blocks. It defines an open-source-oriented boundary for reactive streams, simulation, edge inference and evidence. It is a **proposal**: no upstream runtime, connector, benchmark or production deployment is claimed until an implementation and its acceptance tests are merged.

The extension preserves the existing JFXLCDP evidence chain:
...
The two links use the same name but solve different problems and must not be conflated.

| Reference | Verified capability | JFXLCDP role | Classification and boundary |
|---|---|---|---|
| [Microsoft EdgeML — Bonsai wiki](https://github.com/microsoft/EdgeML/wiki/Bonsai) | Shallow, sparse tree with powerful predictors for binary/multiclass classification, regression and ranking; designed for small-memory, low-latency, offline inference on constrained devices. | Train and evaluate a compact model from simulator or sensor features, then export a reviewed artifact for local inference. | **Research/integration candidate.** The EdgeML repository metadata reports `Other`/`NOASSERTION`; perform file-level license and patent review before redistribution. |
| [`sdk2035/bonsai`](https://github.com/sdk2035/bonsai) | Fork of [Bonsai-Rx](https://github.com/bonsai-rx/bonsai): C#/.NET visual reactive programming with compiler, IDE, standard library and Rx.NET asynchronous observables. | Compose acquisition, feature windows, simulator adapters, visualization, safety rules and replayable workflows. | **Integration candidate.** The fork reports MIT; preserve upstream notices, pin the commit and verify transitive packages. It is not an ML trainer. |
| Project Bonsai (managed service) | Separate hosted machine-teaching product. | Optional provider adapter only, isolated behind a neutral contract. | **External/optional.** Neither repository above establishes this dependency. |
...
flowchart TD
  ...
  B --> C["Versioned feature/event contract"]
  C --> D["Bonsai-Rx reactive graph<br/>sdk2035/bonsai"]
  D --> E["OpenModelica/FMI/ROS 2/MQTT adapters"]
  E --> F["Replayable telemetry and dataset"]
  F --> G["EdgeML Bonsai training/export"]
  G --> H["Edge inference + policy guard"]
...
|---|---|---|
| Specification and AI | Parse requirements, retrieve approved knowledge, generate feature definitions and reviewable workflow changes. | Versioned specifications, citations, typed MCP tools and human approval. |
| Reactive stream | Execute Bonsai-Rx graphs for acquisition, filtering, windowing, feature calculation, visualization and coupling. | Schema validation, back-pressure, bounded queues, deterministic replay and graph revision IDs. |
| Simulation and twin | Run OpenModelica/FMI/ROS 2/MQTT adapters and generate labelled scenarios. | Sandboxed workers, units, scenario manifests and independent assertions. |
| Edge inference | Run the exported EdgeML model and an independent fallback near the sensor or actuator. | Resource budgets, signed artifacts, watchdog, confidence/OOD checks and fail-safe behavior. |
| Evidence and governance | Link requirements, features, data, commits, evaluations, deployed artifacts and decisions. | Immutable audit events, SBOM, SPDX/CycloneDX, retention and rollback. |

## Categorized alternative open-source compendium

The entries below supplement the [Software Dependency Compendium](#software-dependency-compendium). A **candidate** is proposed, not installed or validated by this repository.

| Category | Primary candidate | Open alternative or fallback | JFXLCDP classification | Adoption rule |
|---|---|---|---|---|
| Specification/MBSE | SDD + SysML/Modelica + EMF | Capella/OpenMBEE | Core candidate | Requirements, units and limits remain the source of intent. |
| AI workflow | LangGraph + MCP + local model adapter | Typed state machine without an LLM | Core candidate | Persist graph state, tool calls, checkpoints and review decisions. |
| Visual reactive programming | [`sdk2035/bonsai`](https://github.com/sdk2035/bonsai) | Bonsai-Rx upstream, Node-RED or plain Rx.NET | Integration candidate | Use for stream composition and visualization; not as the learner. |
| Tiny edge learning | [EdgeML Bonsai](https://github.com/microsoft/EdgeML/wiki/Bonsai) | Compact CART baseline, [ONNX Runtime](https://github.com/microsoft/onnxruntime) or [TensorFlow Lite Micro](https://github.com/tensorflow/tflite-micro) | Research/integration candidate | Pass accuracy, memory, latency, energy, robustness and license gates. |
| Feature engineering | Rx.NET operators + versioned Python reference | River or NumPy/SciPy | Integration candidate | Reference and edge paths must agree on units, ordering and missing values. |
| Physics and digital twin | OpenModelica + OMPython + FMI/FMPy | Gazebo, Webots or a domain simulator | Runtime candidate | Version solver, model, parameters, scenario and generated dataset. |
| Device/robot transport | ROS 2, MQTT and typed REST/gRPC | Kafka/NATS for high-throughput backbones | Integration candidate | Normalize transports into one JFXLCDP event envelope. |
| Storage and registry | PostgreSQL + object storage + Qdrant | SQLite/Parquet offline profile | Core candidate | Store specifications, datasets, model cards and evidence with checksums. |
| Observability | OpenTelemetry + Prometheus/Grafana | Structured logs and local metrics | Optional/runtime | Monitor latency, dropped events, drift, confidence and resources. |
| Verification | Deterministic replay, property tests, numerical assertions | Formal or statistical model checking | Test/integration | Do not deploy without a reproducible evidence bundle. |
| Governance | SPDX/CycloneDX SBOM, signed manifest and Git provenance | Reviewed notices for prototypes | Governance/build | Missing or ambiguous license data blocks release. |

## Algorithms and decision patterns

The first row is the referenced EdgeML algorithm. The remaining rows are proposed JFXLCDP patterns that make it useful in a safety-aware workflow.

| Algorithm or pattern | Input/output | Execution point | Acceptance checks |
|---|---|---|---|
| **EdgeML Bonsai** | Fixed-length normalized vector -> class, regression or ranking score. | Workstation/cloud training; device inference. | Task metric, memory/flash, p95 latency, energy and offline behavior. |
| Compact tree baseline | Same feature contract -> interpretable baseline prediction. | Python reference and optional edge build. | Compare errors and resource use with Bonsai and a domain baseline. |
| Reactive windowing | Timestamped events -> ordered windows and derived features. | Bonsai-Rx graph. | No leakage, bounded latency, explicit clock and missing-data policy. |
| Calibration and abstention | Model score + held-out data -> confidence and threshold. | Edge guard or gateway. | Measure false-positive/false-negative cost and abstain below confidence. |
| OOD/anomaly guard | Features, confidence and sensor quality -> accept/abstain/fallback. | Edge or gateway. | Inject noise, missing sensors and unseen scenarios; verify safe fallback. |
| Scenario/curriculum scheduling | Requirement limits + scenarios -> reproducible train/evaluation suites. | JFXLCDP workflow. | Link every scenario to a requirement or hazard and pin its revision. |
| Active-learning queue | Novel/low-confidence observations -> human-label candidates. | Gateway and engineering review. | Never auto-promote labels; preserve reviewer and source evidence. |
| Safety policy/shield | Learned decision + invariants -> allowed action or safe fallback. | Edge and simulator. | Assert hard limits independently of the learned model. |

EdgeML Bonsai is a compact supervised model, not a generative model, planner or safety controller. Learned outputs remain advisory until the independent policy layer and engineering review approve them.

## Versioned data and model contracts

All Bonsai-Rx, simulator and device adapters should normalize to one envelope, regardless of whether the transport is MQTT, ROS 2, REST/gRPC or an in-process observable.
...
The following manifest is a JFXLCDP contract, not an upstream EdgeML API:
...
input: {dtype: float32, shape: [64], normalization: feature-contract-v1}
output: {labels: [normal, warning, emergency], abstention: enabled}
budgets: {max_flash_bytes: requirement-defined, max_ram_bytes: requirement-defined, max_p95_latency_ms: requirement-defined}
provenance: {training_dataset: dataset-revision, source_commit: pinned-edgeml-commit, toolchain: pinned-toolchain, license_review: required}
validation: {replay_suite: suite-revision, baseline: compact-tree-v0, approval: human-review-id}
```

## Proposed connector and MCP boundary

These operation names are JFXLCDP proposals; they are not APIs supplied by either Bonsai repository.

| Operation | Input -> output | Safety boundary |
|---|---|---|
| `bonsai_compile_reactive_graph` | Graph revision -> diagnostics and manifest | Isolated worker; reject undeclared I/O. |
| `bonsai_replay_stream` | Dataset + graph -> deterministic feature stream and timing report | Enforce schema, ordering and resource limits. |
| `bonsai_train_edge_model` | Features + labels + manifest -> candidate model, metrics and model card | No promotion; attach data and code provenance. |
| `bonsai_export_edge_artifact` | Approved model + target -> signed C/C++ or runtime artifact and SBOM | Record compiler, quantization and target profile. |
| `bonsai_evaluate_artifact` | Artifact + replay suite + probe -> accuracy, robustness and resource evidence | Fail closed when a required metric is absent. |
| `bonsai_publish_decision` | Evidence + reviewer decision -> traceability and deploy/rollback event | Authorized human required for safety-critical profiles. |

## End-to-end workflow

1. Capture requirements, units, limits, hazards and acceptance criteria in the existing JFXLCDP specification model.
2. Derive a feature schema and event envelope; obtain engineering review before collecting or generating data.
3. Build a Bonsai-Rx graph for acquisition, filtering, windowing, feature computation and visualization; retain a Python reference path.
...
7. Deploy with a watchdog and safe fallback, collect drift/resource evidence, and send uncertain samples to human review.

## Verification, security and licensing gates

| Gate | Evidence | Release rule |
|---|---|---|
| Functional | Graph compilation, schema/unit checks and deterministic replay. | Block on schema, unit or ordering errors. |
| Predictive | Requirement-linked metrics, calibration, error analysis and baseline comparison. | Block below the requirement-defined threshold. |
| Resource | Measured p95 latency, RAM/flash and energy on target hardware. | Block when any budget is exceeded. |
| Robustness/safety | Missing/noisy/OOD cases, timing jitter, invariants, watchdog and rollback. | Require abstention/fallback and human approval. |
...
| Legal/provenance | File-level notices, SPDX mapping, dependency review and patent/FTO assessment. | Do not distribute unresolved obligations. |

- Sandbox graph compilation and cap CPU, memory, file and network access.
- Sign graph packages, datasets, model manifests and binaries; verify signatures at deployment.
- Keep secrets and private telemetry out of prompts and public indexes; enforce project-level RAG filters.
- Keep a human in the loop for requirement changes, labels, model promotion and deployment.
- `sdk2035/bonsai` reports MIT, but preserve fork and dependency notices and pin the exact commit.
- EdgeML metadata reports `Other`/`NOASSERTION`; do not label its implementation “software libre compatible” without legal review of each file and dependency.
- No architecture statement grants patent freedom; run a jurisdiction-specific FTO review before commercial or safety-critical release.

## Delivery roadmap

| Stage | Deliverables | Exit criterion |
|---|---|---|
| 0 — Governance | ADR separating EdgeML Bonsai and Bonsai-Rx; license register; target hardware profile. | Owners, commits and legal status recorded. |
| 1 — Contracts | Event envelope, feature schema, reactive graph fixture and adapter interface. | Deterministic stream matches the reference path. |
| 2 — Edge baseline | Bonsai candidate, compact baseline, manifest and resource probe. | Metrics and budgets measured on held-out replay. |
| 3 — Simulation/twin loop | OpenModelica/FMI/ROS 2/MQTT fixtures, scenario catalogue and traceability. | Requirement-to-scenario-to-evidence chain reviewable. |
| 4 — Operations | OOD guard, policy shield, signing/SBOM, monitoring and rollback. | All release gates pass with human approval. |
| 5 — Portability | ONNX/TFLM or another permissive fallback and migration tests. | Either Bonsai component can be replaced without changing specifications. |

### References

- [JFXLCDP repository](https://github.com/robotics-intelligent-systems/jfxlcdp)
